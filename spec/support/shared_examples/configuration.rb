...
shared_examples 'configuration_failover_strategy' do
  describe 'failover_strategy' do
    it do
      expect(config.failover_strategy).to be_eql(
        Castle::Failover::Strategy::ALLOW
      )
    end
    ...
    context 'when broken' do
      it do
        expect { config.failover_strategy = :unicorn }.to raise_error(
